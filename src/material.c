...
void bg_model_material_free(struct bg_model_material *mat) {
...
void parse_newmtl_(struct bg_model_material *out, char *line, size_t len) {
...
void parse_color_(union bg_colorf *out, char *line, size_t len) {
  char *initial_line = line;
  line = skip_op_(line, len);
...
void parse_material_line_(struct bg_mtllib *out, char *line, size_t len) {
  if (!len)
    return;
  ...
  if (str_starts_with(line, "#"))
    ...
    struct bg_model_material *mat = bg_vector_at(
        struct bg_model_material, &out->materials, out->materials.length - 1);
    mat->ambient = color;
